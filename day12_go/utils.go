...
func charSliceToIntSlice(sli []string) ([]int, error) {
	// I wish Go had a map function...
	result := make([]int, 0, len(sli))
	for _, val := range sli {
		conv := int(rune(val[0]) - 'a')
		if val == "S" {
			conv = startPlaceholder
		} else if val == "E" {
			conv = endPlaceholder
		}
...
func ReadLinesAsGrid() (Grid, error) {
	...
	for {
		...
		ints, err := charSliceToIntSlice(strings.Split(line, ""))
		if err != nil {
			return Grid{}, err
		}
